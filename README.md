...
- **更新说明：**
  ...
  1. 因为裁剪功能是根据代码结构+数学运算+DOM操作做出来的功能，不是调用什么API就能实现的，所以无法100%保证成功，不过目前经过测试基本上没有问题。

  2. 如果你勾选了智能裁剪，你要确保你的logo中的文字不是编组状态（如果你添加了文字的话）。这里的文字是指在高级编辑页面从右边文本——书法字、标题、列表内容等添加的组件（通过“添加文字”按钮添加的文字不存在这个问题）。为了正常使用智能裁剪功能，你需要单击这些文字，然后点击左上角弹出的取消编组按钮（重复此过程直到所有文字的编组都取消了）。

  3. 如果你勾选了智能裁剪但是有的文字没有取消编组的话，插件会提示你检查是否有未取消编组的文字。其他类型的组件比如图标、素材经过测试不存在上述问题，如果还是提示的话，也请检查图标或者素材是否需要取消编组。总之就是要确保logo里面没有组件是编组状态的。

  4. 如果还存在问题，请提issue反馈~

- V3.2.1新功能：
  1. 支持**登录状态下标智客**的LOGO的裁剪导出（默认不勾选），免去后续自己裁剪的工作，**【使用时注意页面右下角的比例必须为100%】**。
  
- 后续计划新增功能：
  1. 根据LOGO实际大小，导出正方形LOGO
  2. 支持自定义宽度的白边

目前插件满足大部分需求了，如果你希望增加后续功能，**请到issue下面留言**，人多的话再考虑更新~
...
3. 有的LOGO不勾选保留背景但是导出之后还是有背景色，大概率是LOGO本身的原因~
4. 智能裁剪得到的logo可能周边有些细微的空边，这常见于添加了文字素材的logo并且文字素材在logo的边界，这是组件设计时的问题，是正常的现象，如果介意，请再手动裁剪一次~
