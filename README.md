...
**【建议star或者fork，便于日后获取更新~】**

**【你的star和fork是我坚持的动力！！GitHub主页还有很多其他有趣的项目，欢迎关注~~】**

**【如果遇到问题或bug，请提issue~】**

---

### :game_die: V3.0.0已上线！部分功能还是开发中......

- 已完成：
  1. 支持标智客登陆后编辑的LOGO的导出。登陆后，LOGO编辑页面是高级版的编辑页面，相比不登陆情况下具有以下特点：1）可以自由拖动元素；2）可以增加新元素；3）可以增加**付费**背景。
  2. 优化“去除登录框“按钮的显示。只有在需要去除登录框的页面才会显示该按钮。
  3. 插件会显示当前所在的LOGO编辑页面的类型。比如是标小智还是标智客，标智客是登录状态还是未登录状态。
  4. 优化代码
- 预计新增功能：
  1. 支持裁剪，去除无用白边，只保留LOGO部分！
...
- [演示视频(这是v1的演示视频，v2的类似。也可以看下方文字版的使用方法)](https://www.bilibili.com/video/BV1t84y1r71B?share_source=copy_web)
...
1. 如果点击开始处理报错“undefined”，刷新网页，再使用插件即可
2. 编辑LOGO的时候，请不要超出给出框，否则导出的LOGO可能不完整
