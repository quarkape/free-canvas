...
- **裁剪功能不能100%保证成功。如果你勾选了智能裁剪，你要确保你的logo中的文字不是编组状态（如果你添加了文字的话）。这里的文字是指在高级编辑页面从右边文本——书法字、标题、列表内容等添加的组件（通过“添加文字”按钮添加的文字不存在这个问题）。为了正常使用智能裁剪功能，你需要单击这些文字，然后点击左上角弹出的取消编组按钮（重复此过程直到所有文字的编组都取消了）。如果你勾选了智能裁剪但是有的文字没有取消编组的话，插件会提示你检查是否有未取消编组的文字。其他类型的组件比如图标、素材经过测试不存在上述问题，如果存在，也请检查是否需要取消编组**。
- V3.2.0新功能：
